# Docker 커맨드 살펴보기 
docker을 의도한 형태로 구성하고 운영하기 위해서는 docker이 어떠한 커맨드를 지원하는지 살펴보는 것이 필요하다. 이것은 docker 사용자 인터페이스인 CLI(Command Line Interface)의 세부적인 구성을 파악하는 것이 좋다고 판단했다.  

## Docker CLI

아무 생각하지 않고 터미널을 열고 `docker` 커맨드를 입력한다.  
