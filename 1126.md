...
> `--name` 적용
`--name`은 컨테이너의 이름을 명시한다. 만약 명시하지 않는다면, Docker 엔진에서 랜덤한 이름으로 명명한다.

* 이름을 명시하지 않은 경우

```Bash 
$ docker run --name b01 busybox
```

```Bash
$ docker ps -a
CONTAINER ID  IMAGE    COMMAND   CREATED         STATUS                     PORTS  NAMES
177ac385ab9f  busybox  "sh"      13 seconds ago  Exited (0) 12 seconds ago         h01
```
...
본격적인 실습에 진행하기에 앞서 `--name`에 대해 알아보자.
위의 실습에서 보면, `hopeful_dijkstra`라는 것을 볼 수 있다. 이것은 Docker 엔진에서 설정한 컨테이너의 이름으로 명시적으로 컨테이너를 이름을 정의하지 않으면 랜덤하게 이름을 생성한다. 
...
`--rm`은 컨테이너의 이름을 명시한다. 만약 명시하지 않는다면, Docker 엔진에서 랜덤한 이름으로 명명한다.
...
- [x] tty (-t, --tty)
- [ ] volume (-v, --volume list)
- [ ] volume (-p, --publish list)

우리는 이번 실습과정에서 아래의 체크리스트에서 `-d`, `-i`, `-t`, `--rm`, `--name`, `-t`
